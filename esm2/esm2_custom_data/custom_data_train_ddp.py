...
# torchrun --standalone --nnodes=1 --nproc-per-node=8 amgen-train-ddp.py --train_sample_count=50000 --model_id="facebook/esm2_t33_650M_UR50D" --num_epochs=3
...
def main():
    ...
    test_partition = list(range(nTrain,num_lines+1))
    test_dataset = Dataset(test_partition,file_index_map_df)


    train_sampler = ElasticDistributedSampler(train_dataset)

    test_sampler = ElasticDistributedSampler(test_dataset)
